...
type Lingkaran struct { 
    pusat  Titik radius int 
} 
 
func jarak(p, q Titik) float64 { 
    return math.Sqrt(float64((p.x-q.x)*(p.x-q.x) + (p.yq.y)*(p.y-q.y))) } 
...
func posisiTitik(l1, l2 Lingkaran, t Titik) string { 
    dalamL1 := didalamLingkaran(l1, t)     dalamL2 := didalamLingkaran(l2, t)     if dalamL1 && dalamL2 { 
...
func main() {
    ...
    fmt.Println("Masukkan koordinat pusat dan radius lingkaran 2:")     fmt.Scan(&l2.pusat.x, &l2.pusat.y, &l2.radius)     fmt.Println("Masukkan koordinat titik sembarang:")     fmt.Scan(&t.x, &t.y)     hasil := posisiTitik(l1, l2, t)     fmt.Println(hasil) 
} 
