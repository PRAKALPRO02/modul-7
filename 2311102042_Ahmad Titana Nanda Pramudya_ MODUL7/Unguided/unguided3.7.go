...
func main() {
	...
	for i := 1; ; i++ {
		var skorA, skorB int
		fmt.Printf("Masukkan skor pertandingan %d (%s vs %s): ", i, klubA, klubB)
		...
		if skorA > skorB {
			pemenang = append(pemenang, fmt.Sprintf("Hasil %d: %s menang", i, klubA))
		} else if skorB > skorA {
			pemenang = append(pemenang, fmt.Sprintf("Hasil %d: %s menang", i, klubB))
		} else {
